...
my_data = ("Denisova Ekaterina",31,1,2004)
...
attestate ={
            ...
            "Biology": 5,
            "Visual art": 5,
            "Basics of life safety": 5,
            "Russian language": 5,
            "English language": 4
            }

attestate_list =list(attestate)
family_names =['Ирина','Павел','Дмитрий','Алексей','Екатерина','Анастасия']
...
things = queue.Queue()
subject = queue.Queue()
k = int(input("Введите номер предмета"))
while k!=-1:
    if(k<1 or k>15):
...
while k!=-1:
    ...
    k = int(input("Для выхода -1, иначе любое число: "))
...
n = int(input("#11\nВведите начальное число последовательности: "))
