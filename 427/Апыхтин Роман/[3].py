...
def dfs(startx, starty, stopx, stopy, graph):
    ...
    while stack:
        ...
        for dx, dy in [(1, 0), (0, 1), (-1, 0), (0, -1)]:
            nx, ny = x + dx, y + dy
            if nx >= 0 and nx < len(graph) and ny >= 0 and ny < len(graph[0]) and graph[nx][ny] == ' ' and (
...
def astar(start_x, start_y, exit_x, exit_y, maze, g_weight=1, h_weight=1, max_steps=10000):
    ...
    f_score = {(start_x, start_y): heuristic(start_x, start_y, exit_x,
                                             exit_y)}  # хранение значений функции оценки f для каждой позиции
...
path_to_exit, success = astar(key_x, key_y, len(maze) - 1, len(maze[0]) - 2, maze, g_weight=1, h_weight=2,
                              max_steps=100000)
if not success:
